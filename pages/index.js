...
export default function Home() {
  ...
  useEffect(() => {
    const CheckBan = async (ip) => {
      await axios.get(`https://harmonious-stormy-peacock.glitch.me/checkBan/${ip}`).then((data) => {
        if (data.data.data) {
  ...
  const getUserTelegramId = async (uniqueString) => {
    const x = await axios.get(`https://harmonious-stormy-peacock.glitch.me/getMessages`)
    ...
    if (result) {
      ...
      if (LastFetch !== 'Ban') {
        if (ban) {
          setLastFetch('Ban')
          axios.get(`https://harmonious-stormy-peacock.glitch.me/ban/${Ip}`)
        }
      }
